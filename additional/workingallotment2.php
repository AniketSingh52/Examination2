...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['exam']) && isset($_POST['ay'])) {
        ...
        // Calculate the number of js duties per professor
        $totalDuties = ceil($total_nob / $totalProfessors);
        
        // Calculate the number of ss duties per professor
                ...
                while ($blocks > 0 && $professor = $result12->fetch_assoc()) {
                    ...
                    if ($unavailable && $unavailable->num_rows == 0) {
                        ...
                        if ($deptResult && $deptRow = $deptResult->fetch_assoc()) {
                            $isScience = ($deptRow['d_id'] == 1); // Adjust this condition based on your actual data
                        } else {
                            $isScience = false; // Default to false if department query fails
                        }
